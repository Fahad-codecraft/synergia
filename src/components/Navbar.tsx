    ...
    >
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <item.icon 
                    className={`w-5 h-5 transition-colors duration-300 ${
                      activeItem === item.path ? 'text-red-500' : 'text-gray-400 group-hover:text-purple-400'
                    }`}
                  />
                  <span className={`relative z-10 font-medium transition-colors duration-300 ${
                    activeItem === item.path ? 'text-white' : 'text-gray-400 group-hover:text-white'
                  }`}>
                    {item.name}
                  </span>
                </div>
                ...
                {/* Bottom line */}
                <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-gradient-to-r from-red-500 to-purple-500 transition-all duration-300 ${
                  activeItem === item.path ? 'w-full opacity-100' : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-100'
                }`}></div>
              </Link>
            ))}
          </div>
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-r from-red-500/10 to-purple-500/10 rounded-xl transition-transform duration-300 ${
            ...
            {navItems.map((item) => (
              ...
              >
                <item.icon className={`w-5 h-5 ${
                  activeItem === item.path ? 'text-red-500' : 'text-gray-400'
                }`} />
                {item.name}
              </Link>
            ))}
